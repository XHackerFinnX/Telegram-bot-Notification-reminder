...
class NotifyInlineAdd1h(StatesGroup):
...
async def reg_comment(message: types.Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        await NotifyInlineAdd1h.check.set()
...
async def reg_check(message: types.Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        if data['check'] == "Да ✅":
            ...
            time_interval_1h = 1 * 60 * 60
            await main_push(message.chat.id, time_interval_1h, data['comment'])
...
async def start_inline_add1h(dp: Dispatcher):
    
    await NotifyInlineAdd1h.comment.set()
    
    dp.register_message_handler(reg_comment, state=NotifyInlineAdd1h.comment)
    dp.register_message_handler(reg_check, state=NotifyInlineAdd1h.check)