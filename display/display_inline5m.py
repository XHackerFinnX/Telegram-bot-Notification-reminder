...
class NotifyInlineAdd5m(StatesGroup):
...
async def reg_comment(message: types.Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        await NotifyInlineAdd5m.check.set()
...
async def reg_check(message: types.Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        if data['check'] == "Да ✅":
            ...
            time_interval_5m = 5 * 60
    
            await main_push(message.chat.id, time_interval_5m, data['comment'])
...
async def start_inline_add5m(dp: Dispatcher):
    
    await NotifyInlineAdd5m.comment.set()
    
    dp.register_message_handler(reg_comment, state=NotifyInlineAdd5m.comment)
    dp.register_message_handler(reg_check, state=NotifyInlineAdd5m.check)